        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">تعديل شهادة</h3>
                    ...
                    <form action="{{ route('certificates.update', $certificate) }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('employee_id') is-invalid @enderror" 
                                        ...
                                        @foreach($employees as $employee)
                                            <option value="{{ $employee->id }}" {{ old('employee_id', $certificate->employee_id) == $employee->id ? 'selected' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('certificate_type_id') is-invalid @enderror" 
                                        ...
                                        @foreach($certificateTypes as $type)
                                            <option value="{{ $type->id }}" {{ old('certificate_type_id', $certificate->certificate_type_id) == $type->id ? 'selected' : '' }}>
                                                {{ $type->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('specializaion_id') is-invalid @enderror" 
                                        ...
                                        @foreach($specializations as $specialization)
                                            <option value="{{ $specialization->id }}" {{ old('specializaion_id', $certificate->specializaion_id) == $specialization->id ? 'selected' : '' }}>
                                                {{ $specialization->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('country_id') is-invalid @enderror" 
                                        ...
                                        @foreach($countries as $country)
                                            <option value="{{ $country->id }}" {{ old('country_id', $certificate->country_id) == $country->id ? 'selected' : '' }}>
                                                {{ $country->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('thesis_title') is-invalid @enderror" 
                                           id="thesis_title" name="thesis_title" 
                                           value="{{ old('thesis_title', $certificate->thesis_title) }}"
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('company') is-invalid @enderror" 
                                           ...
                                           value="{{ old('company', $certificate->company) }}"
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control @error('release-date') is-invalid @enderror" 
                                           ...
                                           value="{{ old('release-date', $certificate->{'release-date'} ? \Carbon\Carbon::parse($certificate->{'release-date'})->format('Y-m-d') : \Carbon\Carbon::now()->format('Y-m-d')) }}" 
                                           required>
                                    @error('release-date')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    @if($certificate->certificate_file)
                                        <div class="mt-2">
                                            <span class="badge badge-success">ملف مرفق موجود</span>
                                            <a href="{{ route('certificates.download', $certificate) }}" 
                                               class="btn btn-sm btn-outline-success ml-2">
                                                <i class="fas fa-download"></i> تحميل الملف الحالي
                                            </a>
                                        </div>
                                    @else
                                        <span class="text-muted">لا يوجد ملف مرفق حالياً</span>
                                    @endif
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      placeholder="أدخل وصفاً للشهادة">{{ old('description', $certificate->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="datetime-local" class="form-control @error('date_created') is-invalid @enderror" 
                                           ...
                                           value="{{ old('date_created', $certificate->date_created ? \Carbon\Carbon::parse($certificate->date_created)->format('Y-m-d\TH:i') : \Carbon\Carbon::now()->format('Y-m-d\TH:i')) }}">
                                    @error('date_created')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="form-group">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> تحديث الشهادة
                            </button>
                            <a href="{{ route('certificates.index') }}" class="btn btn-secondary">
                                <i class="fas fa-times"></i> إلغاء
                            </a>
                            <a href="{{ route('certificates.show', $certificate) }}" class="btn btn-info">
